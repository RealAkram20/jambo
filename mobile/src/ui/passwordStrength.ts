/**
 * A password strength estimate.
 *
 * **Ported from Kangaru's `mobile/src/auth/passwordStrength.ts`, not
 * reinvented.** Rio asked for "the strength helper as we did in Kangaru" on
 * 2026-09-10, and the module there is already argued through — two roads to a
 * full bar, obvious junk named rather than scored, and a visible scale. Copying
 * it and changing the floor is a smaller and more honest change than writing a
 * second opinion about the same problem.
 *
 * What changed in the port, and only this: the floor is **8**, which is what
 * `Password::defaults()` holds on this server; the dictionary below names
 * Jambo rather than Kangaru; and the wording says viewer rather than viewer.
 * The scoring, the levels and the reasoning are Kangaru's.
 *
 * Deliberately a **guide, not a gate**. Jambo's server rule is the length and
 * nothing else — no mixed case, no digit, no symbol — so a form that demanded
 * them would refuse passwords the platform accepts and teach people to write
 * "Password1!", which every cracking dictionary already holds. Length is
 * weighted above character variety, and obvious junk is called out by name.
 *
 * **The meter has to be readable as a rule, not as a mood.** Every segment is
 * earned by one stated thing, and the checklist says what those things are. A
 * viewer who watches the bar move should be able to say why it moved.
 *
 * Not zxcvbn: ~400KB of frequency tables is a heavy thing to put in a bundle
 * that downloads over Ugandan mobile data.
 */

/**
 * `Illuminate\Validation\Rules\Password::defaults()` on this server, read
 * rather than assumed: **min 8, and no complexity requirement at all** —
 * mixedCase, numbers, symbols and uncompromised are every one of them off.
 *
 * That is what makes the philosophy below the right one rather than a
 * borrowed opinion. The server's only hard rule is the length, so the meter
 * must be a guide; a form that refused a long passphrase for having no digit
 * would be inventing a rule Jambo does not hold, and rejecting passwords the
 * platform accepts.
 */
...
export type PasswordRequirementKey = 'length' | 'case' | 'number' | 'symbol' | 'guessable';
...
export type PasswordStrength = {
   ...
   * invisible is a bar that grades against a standard the viewer was never
 ...
 * it on the checklist alone would tell a viewer to ruin it.
 */
const LENGTH_STEPS = [
 ...
 * "Fair" and was told to grow, while the viewer had no way to learn what the
 * remaining two segments wanted. Requirements a viewer can see beat a scale
 * they cannot.
 */
function requirementsFor(password: string): PasswordRequirement[] {
  return [
    ...
    /*
     * 🔴 **The fifth rule, and it was being graded in secret until
     * 2026-09-10.**
     *
     * Rio typed `Jambo2026@N`, watched all four boxes go green, and the bar
     * still read Weak on one segment out of four. He asked the only question
     * that matters: *how does the person know?* They could not. The cap came
     * from `predictable()`, which is a rule the checklist never listed, so the
     * screen showed four reasons to be confident and one contradictory verdict
     * with no way to connect them.
     *
     * That is precisely the failure this module's own header warns about — a
     * bar whose standard is invisible grades against something nobody was
     * told, which is a verdict rather than a guide. Kangaru has the same hole;
     * it just needs a dictionary hit to show, and nobody there had typed one.
     *
     * So the hidden rule becomes a visible one. It does not earn a segment —
     * it is a gate, and `predictable` still caps the score at one — but it now
     * appears unticked beside the four that passed, which is the difference
     * between "Weak, and here is the rule you broke" and "Weak, because."
     */
    {
      key: 'guessable',
      label: 'Not an obvious guess',
      met: password.length > 0 && predictable(password) === null,
    },
  ];
}
 ...
 *
 * Unanchored on purpose. "Jambo2024!" was already caught; "myJambo2024"
 ...
 */
const OBVIOUS = /(password|passw0rd|welcome|qwerty|letmein|admin|kampala|uganda|jambo|films|abc123|1234)/i;
 ...
 * length it is the single likeliest thing a hurried viewer types.
 */
...
export function passwordStrength(password: string): PasswordStrength {
  ...
  if (password.length < MINIMUM_PASSWORD_LENGTH) {
    ...
    return {
      ...
      // something a viewer can act on without re-counting what they typed.
   ...
   *
   * The checklist is the one a viewer can act on: four named things, one
   ...
   * characters can be. Scoring it on the checklist alone would tell a viewer
   * to wreck a good passphrase to satisfy a tick.
   *
   * Both count upwards only, so the bar cannot fall while somebody is typing.
   */
  /*
   * The first four only. The fifth is a gate rather than a segment — counting
   * it would let a password earn a bar for *not* being on a list, and a
   * guessable one is capped at one segment below regardless.
   */
  const met = requirements.filter(
    (requirement) => requirement.key !== 'guessable' && requirement.met,
  ).length;
 ...
 * carried it — but the viewer cannot know that from the ticks, so the sentence
