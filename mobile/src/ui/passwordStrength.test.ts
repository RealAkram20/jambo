import { MINIMUM_PASSWORD_LENGTH, passwordStrength, STRENGTH_SEGMENTS } from './passwordStrength';
 ...
 * standard the viewer cannot see is a verdict, not a guide.
 *
...
it('shows the five rules it grades, met or not, so the scale is never hidden', () => {
  expect(metKeys('Kim27!ne')).toEqual(['length', 'case', 'number', 'symbol', 'guessable']);

  // Nine lower-case letters: long enough to send, and nothing else.
  expect(metKeys('kimberley')).toEqual(['length', 'guessable']);
});

/**
 * 🔴 **The regression Rio found on a device, 2026-09-10.**
 *
 * He typed `Jambo2026@N`. Every one of the four visible boxes went green — the
 * length, the mixed case, the digit, the symbol — and the bar still read Weak
 * on one segment of four. His question was the whole of the problem: *how does
 * the person know?*
 *
 * They could not. The cap came from the dictionary check, which was a real
 * rule doing real work and was not on the checklist, so the screen showed four
 * reasons to be confident and one verdict that contradicted them with nothing
 * joining the two.
 *
 * The score was never wrong — the platform's own name plus a year plus a
 * symbol is the first thing anybody guesses. Only the explanation was missing.
 * So the hidden rule is a visible one now: unticked, named, and sitting beside
 * the four that passed.
 */
it('shows the broken rule when a password ticks every other box and still scores weak', () => {
  const strength = passwordStrength('Jambo2026@N');

  // The verdict is unchanged, because it was right.
  expect(strength.score).toBe(1);
  expect(strength.level).toBe('weak');

  // And now it is legible: four met, and the one that caps it visibly unmet.
  expect(metKeys('Jambo2026@N')).toEqual(['length', 'case', 'number', 'symbol']);

  const guessable = strength.requirements.find((rule) => rule.key === 'guessable');
  expect(guessable?.met).toBe(false);
  expect(guessable?.label).toBe('Not an obvious guess');

  // The sentence still names which guess it is, rather than only that it is one.
  expect(strength.hint).toBe('That contains one of the first passwords anyone would guess.');
});

/**
 * The gate does not hand out segments.
 *
 * A password that is merely *not* on the dictionary must not earn a bar for
 * it — the fifth rule caps, it never credits. Mutation check: count
 * `guessable` among the met requirements and a two-rule password reads three.
 */
it('does not let the fifth rule earn a segment', () => {
  // Length and case only, and not a dictionary word.
  expect(passwordStrength('Kimberley').score).toBe(2);
});

it('counts the rules a viewer has met, one segment each', () => {
  // length only.
...
it('uses the one floor this app already has, rather than a second copy', () => {
  ...
  // seven characters while the submit rule refused them would tell a viewer
 ...
 * alone would tell a viewer to wreck it.
 */
...
it('catches the platform s own name wherever it appears, not only at the start', () => {
  expect(passwordStrength('JamboRide2024').score).toBe(1);
  expect(passwordStrength('myJambo2024!!').score).toBe(1);
...
it('gives every level a word, because colour alone carries no meaning', () => {
  ...
  // colour-blind viewer and to anyone glancing at a phone in the sun.
