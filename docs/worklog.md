...
### 2026-09-12 — self-hosting withdrawn; 1.8.41 broke the live file manager

**Status:** complete, 1.8.42. This undoes the live part of the entry above.

**What broke.** Drag-and-drop and the right-click actions stopped working on
production after 1.8.41. The gallery's page declares fourteen assets; its
320 KB bundle lazy-loads **twelve more packages** on first use — uppy, plyr,
hls.js, codemirror (plus a syntax file per language), pannellum, jsmediatags,
headroom, marked, marked-base-url, intersection-observer, dayjs locales, flag
icons. Repointing the asset path without them 404s every one, and a 404 on a
lazily-injected `<script>` is silent.

**Why my tests passed anyway.** They read what `index.php` declares. The real
list lives inside a minified bundle. The verification was aimed at the wrong
artefact, which is a more useful thing to know than "I missed a file".

**What 1.8.42 does.** Stops enforcing `assets`, and **actively deletes** it
from any config that has it — not enforcing is not enough, because every box
that took 1.8.41 already has the line written down. The vendored files stay,
staged. A new test permits only `assets` absent, or `assets` present with every
package the bundle names vendored; it reads that list from the bundle, so the
half-state cannot be expressed again.

**Rio's server was fixed by hand first** (deleting the line from
`_files/config/config.php`) so his team got their features back immediately,
before any of this was written.

**What finishing the job needs**, if it is ever wanted: vendor the remaining
packages — about five megabytes including CodeMirror's per-language modes and
the flag icons — generated by a script that reads the bundle, never a hand
list. The test above already refuses anything less.

**Two process notes, both mine.**

- I broke production by verifying the wrong artefact and shipping on a green
  suite. The suite was green; it was pointed at the wrong thing.
